	...
	if (!color) {
		...
		for(x=0;x<width-1;x++){
			for(y=0;y<height-1;y++){
				Y=videoFrame[y*width+x];
				SET_PIXEL32(x,y,RGB32(Y,Y,Y));
			}
		}
		...
		/* A COMPLETER */
		int k,m;
		int width_420=width/2;
		int height_420=height/2;
		int size_420=height_420*width_420;
		...
		for(x=1;x<width;x++){
			...
			for(y=1;y<height;y++){
				...
				SET_PIXEL32(x-1,y-1,yuv_to_rgb_24(videoFrame[(y-1)*width+(x-1)],videoFrame[size+size_420+(m-1)*width_420+k-1],videoFrame[size+(m-1)*width_420+k-1]));
			}
		}
	}
}
...
int detec_contour(uint8_t* videoFrame, int width, int height ){
	...
	uint32_t Y_val;
	for  (i =0; i<size; i++){
		Y[i] = (int)videoFrame[i];
	}

	for (i =1; i< width-1;i++){
		for (j = 1; j < height; j++){
			Y_val = (uint32_t)abs(Y[(j+1)*width+i-1]+Y[(j+1)*width +i] + Y[(j+1)*width+i+1] -Y[(j-1)*width+i-1] -Y[(j-1)*width+i] -Y[(j-1)*width+ i + 1]) + abs(Y[(j-1)*width+i+1] +Y[(j)*width+i+1] + Y[(j+1)*width+i+1] -Y[(j-1)*width+i-1] -Y[(j)*width+i-1] -Y[(j+1)*width+i-1]);
			if (Y_val < 200) Y_val = 0;
			else Y_val = 255;
			SET_PIXEL32(i,j,RGB32(Y_val,Y_val,Y_val));
		}
	}
...
int detec_contour_updated(uint8_t* videoFrame, int width, int height ){
	int i,j;
	int size = width*height;
	int Y [size] ;
...
int detec_contour_filtre(uint8_t* videoFrame, int width, int height ){
	int i,j;
	int size = width*height;
	...
	uint32_t l1_gauss,l2_gauss,l3_gauss,l4_gauss,l5_gauss;
	...
	for (i = 2; i< width-3;i++){
		for (j = 2; j < height-3; j++){
                        l1_gauss = 2*Y_val[(j-2)*width+i-2] +4*Y_val[(j-2)*width+i-1]  +5*Y_val[(j-2)*width+i] +4*Y_val[(j-2)*width+i+1] +2*Y_val[(j-2)*width+i+2];
                        l2_gauss = 4*Y_val[(j-1)*width+i-2] +9*Y_val[(j-1)*width+i-1] +12*Y_val[(j-1)*width+i] +9*Y_val[(j-1)*width+i+1] +4*Y_val[(j-1)*width+i+2];
                        l3_gauss = 5*Y_val[(j)*width+i-2]  +12*Y_val[(j)*width+i-1]   +15*Y_val[(j)*width+i]  +12*Y_val[(j)*width+i+1]   +5*Y_val[(j)*width+i+2];
                        l4_gauss = 4*Y_val[(j+1)*width+i-2] +9*Y_val[(j+1)*width+i-1] +12*Y_val[(j+1)*width+i] +9*Y_val[(j+1)*width+i+1] +4*Y_val[(j+1)*width+i+2];
                        l5_gauss = 2*Y_val[(j+2)*width+i-2] +4*Y_val[(j+2)*width+i-1]  +5*Y_val[(j+2)*width+i] +4*Y_val[(j+2)*width+i+1] +2*Y_val[(j+2)*width+i+2];
                        Y_val[j*width+i] = (uint32_t) (l1_gauss+l2_gauss+l3_gauss+l4_gauss+l5_gauss)/159;
                }
        }
        ...
        for (i = 1; i< width-2;i++){
			...
			l1_lap = -Y_val[(j-1)*width+i-1]  -Y_val[(j-1)*width+i] -Y_val[(j-1)*width+i+1];
                        ...
                        Y_val[j*width+i] = (uint32_t)(Y_val[j*width+i]-lambda*laplacien);
		}
	}
    ...
    // Gradient d'intensité
        for (i = 1; i< width-1;i++){
                for (j = 1; j < height-1; j++){
                        ...
                        if (Grad[j*width+i] > max_grad) max_grad = Grad[j*width+i];                        
		}
	}
	...
	for (i = 1; i< width-2;i++){
		for (j = 1; j < height-1; j++){
			            val = Grad[j*width+i]*255/max_grad;
                        if (val < 200) val = 0;
...
int superpo_img(uint8_t* videoFrame, int width, int height){
	int x,y;
	int size = width*height;
	uint8_t Y_val,Y_contour;
	int Y [size];
	...
	if (!color) {
		...
		for(x=0;x<width-1;x++){
			for(y=0;y<height-1;y++){
				Y_val=videoFrame[y*width+x];
				Y_contour = (uint8_t)abs(Y[(y+1)*width+x-1]+Y[(y+1)*width +x] + Y[(y+1)*width+x+1] -Y[(y-1)*width+x-1] -Y[(y-1)*width+x] -Y[(y-1)*width+ x + 1]) + abs(Y[(y-1)*width+x+1] +Y[(y)*width+x+1] + Y[(y+1)*width+x+1] -Y[(y-1)*width+x-1] -Y[(y)*width+x-1] -Y[(y+1)*width+x-1]);
				if (Y_contour < 200) Y_val = 0;
				else Y_contour = 255;
				SET_PIXEL32(x,y,RGB32(Y_val+Y_contour,Y_val+Y_contour,Y_val+Y_contour));
			}
		}
		...
		for(x=1;x<width;x++){
			if(x%2==0) k=x/2;
			for(y=1;y<height;y++){
				Y_val = (uint32_t)abs(Y[(y+1)*width+x-1]+Y[(y+1)*width +x] + Y[(y+1)*width+x+1] -Y[(y-1)*width+x-1] -Y[(y-1)*width+x] -Y[(y-1)*width+ x + 1]) + abs(Y[(y-1)*width+x+1] +Y[(y)*width+x+1] + Y[(y+1)*width+x+1] -Y[(y-1)*width+x-1] -Y[(y)*width+x-1] -Y[(y+1)*width+x-1]);
				if(x==1 && y==1){ k=1;
						          m=1;}
				if(y%2==0) m=y/2;
				
				if (Y_val > 180) SET_PIXEL32(x-1,y-1,RGB32(255,0,0));
				else SET_PIXEL32(x-1,y-1,yuv_to_rgb_24(videoFrame[(y-1)*width+(x-1)],videoFrame[size+size_420+(m-1)*width_420+k-1],videoFrame[size+(m-1)*width_420+k-1]));
...
int main(int argc, char *argv[])
{
	...
	ProcessFunc			proc_func= detec_contour_filtre;//detec_contour;//process_image_yuv420p;
	int					factor = 6;
