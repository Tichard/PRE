...
uint8_t checksum(uint8_t* frame, unsigned long size)
...
// switch case regarding the frame type
{
	...
	// send the message	
	sendXbee(serial_fd,type,msg,size);	
	
	return 0;
}


int sendXbee(int serial_fd, int type, uint8_t* msg, unsigned long size)
	...
	if (read_header[0] == 0x7E)
	{
		...
		switch (read_data[0])
		{
			...
			case 0x90 : // ZigBee receive packet
			{
				...
				if(checksum(msg, totSize-1)!= read_data[totSize-1])
				{
				...
				//get value
				for (i=0; i<totSize-15; i++)
				{
					return_data |= read_data[14 + i]<<(8*i);
				}
				...
				{				
					...
					{					
						...
						// if it's a "unit" transmitting
						if (read_data[13] == 0xFF)
						{
							sprintf( addr[i].unit[read_data[12]], "%c%c%c",read_data[14], read_data[15], read_data[16]);
				...
				sprintf(data, "%d,%d,%u,%s",i, read_data[12], return_data,addr[i].unit[read_data[12]]);
				return strlen(data);
